...
public class GameOverPopup extends Scene{
	...
	public GameOverPopup(IGameContext context, DialogResultListener resultCallback) {
		super(context);
	...
	private void close(){
		...
		Label label = (Label)elements.get(2);
	...
	private void setupUI() {
		...
		se.skoggy.ui.Label label = uiFactory.createLabel("Game Over!", 200f, 200f);
		label.setScale(0f);
		
		MessageBox box = uiFactory.createMessageBox(new Vector2(width * 0.5f, height * 0.5f), 300f);
		
		TouchButton btnYes = uiFactory.createRoundIconButton("restart", "yellow", 600);

		label.setPosition(width * 0.5f, height * 0.5f);
		btnYes.setPosition(width * 0.5f, height * 0.72f);
